...
public class Ex02_ClassStudent {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < s.length; i++) {
			System.out.println(i+1+"\t"+s[i].getName()+"\t"+s[i].getJava()+"\t"+s[i].getSpring()+"\t"+s[i].getTotal()+"\t"+s[i].getAverage()+"\t"+s[i].getGrade());
		}
		...
		s[1].setSpring(99);
		System.out.println("2번 학생의 자바점수만 수정");
		s[2].setJava(60);
		System.out.println("수정된 데이터로 다시 출력");
		System.out.println("번호\t이름\t자바\t스프링\t총점\t평균\t등급");
		System.out.println("=".repeat(60));
		for(int i = 0; i < s.length; i++) {
			System.out.println(i+1+"\t"+s[i].getName()+"\t"+s[i].getJava()+"\t"+s[i].getSpring()+"\t"+s[i].getTotal()+"\t"+s[i].getAverage()+"\t"+s[i].getGrade());
